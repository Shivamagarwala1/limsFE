...
import { IoMdAdd, IoMdCloseCircleOutline, IoMdImages, IoMdMenu } from 'react-icons/io';
import { useSelector } from 'react-redux';
import { CiCalendarDate } from "react-icons/ci";
import { employeeMasterHeaderList } from '../../../listData/listData';
import { getAllCentreApi, getAllCityApi, getAllDepartmentApi, getAllDistrictApi, getAllEmployeeDataApi, getAllEmpTitleApi, getAllRoleApi, getAllStateApi, getAllZoneApi, getDesignationApi, getSingleEmployeeDataUsingEmpId, saveCityApi, saveDistrictApi, saveEmployeeDataApi, saveStateApi, updateEmployeeStatusDataApi } from '../../../../service/service';
...
export default function EmployeeMaster() {
    ...
    const [employeeData, setEmployeeData] = useState({
        ...
        email: '',
        mobileNo: '',
        dob: new Date('1970-01-01T00:00:00:00Z'.replace(/:\d+Z$/, 'Z')).toISOString(),
        qualification: '',
        bloodGroup: '',
        designationId: 0,
        userName: '',
    ...
    useEffect(() => {
        async function getAllEmpTitle() {
            await getAllEmpTitleApi().then((resp) => {
                setTitle(resp);
            }).catch((err) => {
                toast.error(err?.message);
            })
        ...
        async function getDesignation() {
            await getDesignationApi().then((resp) => {
                setDesignation(resp);
            }).catch((err) => {
                toast.error(err.message);
            })
        ...
        async function getAllDepartment() {
            ...
            await getAllDepartmentApi().then((resp) => {
        ...
        async function getAllCentre() {
            ...
            await getAllCentreApi().then((resp) => {
    ...
    const validateForm = () => {
        ...
        if (!employeeData.addEmpDepartmentAccess || employeeData.addEmpDepartmentAccess.length === 0) {
            errors.addEmpDepartmentAccess = true;
        }

        if (!employeeData.addEmpCentreAccess || employeeData.addEmpCentreAccess.length === 0) {
            errors.addEmpCentreAccess = true;
        }

        if (!employeeData.addEmpRoleAccess || employeeData.addEmpRoleAccess.length === 0) {
    ...
    const onSubmitSaveEmployeeMaster = async (event) => {
        ...
        await saveEmployeeDataApi(updatedFormData).then((resp) => {
            if (resp?.success) {
                toast.success(resp?.message);
    ...
    const handleTheUpdateStatusMenu = async () => {
        ...
        await updateEmployeeStatusDataApi(clickedRowId?.empId, newValue, userId).then((resp) => {
    ...
    const getSingleMenuDataForUpDate = async (empId) => {
        ...
        await getSingleEmployeeDataUsingEmpId(empId).then((resp) => {
            ...
            if (resp?.success) {
                ...
                setConfirmPass(// Retain existing state values
                ...
                setSelectedSearchDropDownData((preventData) => ({
                    ...preventData,
                    designationId: allDesignation.filter((item) => item?.id === resp?.data?.designationId)[0]?.designationName,
                    state: allStateData.filter((item) => item?.id === resp?.data?.state)[0]?.state || 0,
                    district: allDistrictData.filter((item) => item?.id === resp?.data?.district)[0]?.district || 0,
                    city: allCityData.filter((item) => item?.id === resp?.data?.city)[0]?.cityName || 0,
                    defaultcentre: filteredCenters.filter((item) => item?.centreId === resp?.data?.defaultcentre)[0]?.companyName || 0,
                    defaultrole: filteredRoles.filter((item) => item?.id === resp?.data?.defaultcentre)[0]?.roleName || 0,
                }))
    ...
    const filterAllDesignationData = allDesignation?.filter((data) =>
        data?.designationName?.toLowerCase().includes(String(employeeData?.designationId || '').toLowerCase())
    );



    const filterAllStateData = allStateData?.filter((data) =>
        data?.state?.toLowerCase().includes(String(employeeData?.state || '').toLowerCase())
    );

    const filterAllDistrictData = allDistrictData?.filter((data) =>
        data?.district?.toLowerCase().includes(String(employeeData?.district || '').toLowerCase())
    );

    const filterAllCityData = allCityData?.filter((data) =>
        data?.cityName?.toLowerCase().includes(String(employeeData?.city || '').toLowerCase())
    );


    const filterAllDefaultCenterData = allDefaultCenter?.filter((data) =>
        (data?.companyName?.toLowerCase() || '').includes(String(employeeData?.defaultcentre || '').toLowerCase())
    );


    const filterAllDefaultRoleData = allDefaultRole?.filter((data) =>
        data?.roleName?.toLowerCase().includes(String(employeeData?.defaultrole || '').toLowerCase())
    );
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-2  mt-2 mb-1  mx-1 lg:mx-2">
                    ...
                    <div className="relative flex-1">
                        <input
                            ...
                            onChange={handelChangeEmployeeDetails}
                            placeholder=" "
                            className={`inputPeerField peer ${formErrors.email ? "border-b-red-500" : "border-borderColor"}  focus:outline-none`}
                        />
                    ...
                    <div className="relative flex-1">
                        <input
                            ...
                            id="designationId"
                            name="designationId"
                            value={selectedSearchDropDownData?.designationId || employeeData?.designationId || ''}
                            onChange={(e) => {
                            ...
                            placeholder=" "
                            className={`inputPeerField peer ${formErrors.designationId ? "border-b-red-500" : "border-borderColor"
                                }  focus:outline-none`}
                        />
                    ...
                    <div className="relative flex-1">
                        <input
                            ...
                            onChange={handelChangeEmployeeDetails}
                            placeholder=" "
                            className={`inputPeerField peer ${formErrors.userName ? "border-b-red-500" : "border-borderColor"} focus:outline-none`}
                        />
                    ...
                    <div className='flex items-center gap-[0.25rem]'>
                        ...
                        <div className="relative flex-1">
                            <input
                                ...
                                onChange={handelChangeEmployeeDetails}
                                placeholder=" "
                                className={`inputPeerField peer ${formErrors.password ? "border-b-red-500" : "border-borderColor"} focus:outline-none`}
                            />
                        ...
                        <div className="relative flex-1">
                            <input
                                ...
                                placeholder=" "
                                className={`inputPeerField peer ${formErrors.confirmPass ? "border-b-red-500" : "border-borderColor"}  focus:outline-none`}
                            />
                    ...
                    <div className='relative flex-1 flex items-center gap-[0.20rem] w-full justify-between'>
                        <div className="relative flex-1">
                            <input
                                ...
                                id="state"
                                name="state"
                                value={selectedSearchDropDownData?.state || employeeData.state || ''}
                                onChange={(e) => {
                                    handelChangeEmployeeDetails(e)
                    ...
                    <div className='relative flex-1 flex items-center gap-[0.20rem] w-full justify-between'>
                        <div className="relative flex-1">
                            <input
                                ...
                                id="district"
                                name="district"
                                value={selectedSearchDropDownData?.district || employeeData.district || ''}
                                onChange={(e) => {
                                    handelChangeEmployeeDetails(e)
                    ...
                    <div className="relative flex-1">
                        <div
                            className={`flex peer items-center border-[1.5px] ${formErrors.addEmpDepartmentAccess ? "border-b-red-500" : "border-borderColor"} rounded text-xxxs h-[1.6rem] text-[#495057] bg-white`}
                            onClick={() => showSearchBarDropDown !== 5 ? openShowSearchBarDropDown(5) : openShowSearchBarDropDown(0)}
                        >
                            <input
                                type="text"
                            ...
                            <div>
                                {
                                    showSearchBarDropDown === 5 ? <RiArrowDropUpLine className='text-xl cursor-pointer' /> : <RiArrowDropDownLine className='text-xl cursor-pointer' />
                                }
                            </div>
                                                    ...
                                                    >
                                                        <div>
                                                            <input
                                                                ...
                                                                onChange={(e) => handleCheckboxChange(e, data, 'deptName')}
                                                            />
                                                        </div>
                    ...
                    <div className="relative flex-1">
                        <div
                            className={`flex peer items-center border-[1.5px] ${formErrors.addEmpCentreAccess ? "border-b-red-500" : "border-borderColor"} rounded text-xxxs h-[1.6rem] text-[#495057] bg-white`}
                                                ...
                                                allCenterData?.map((data, index) => (
                                                    ...
                                                    >
                                                        <div>
                                                            <input
                                                                type="checkbox"
                                                                checked={employeeData?.addEmpCentreAccess?.some((item) => item.centreId === data.centreId)}
                                                                onChange={(e) => handleCheckboxChange(e, data, 'centerData')}
                                                            />
                                                        </div>
                    ...
                    <div className="relative flex-1">
                        <div
                            className={`flex peer items-center border-[1.5px] ${formErrors.addEmpRoleAccess ? "border-b-red-500" : "border-borderColor"} rounded text-xxxs h-[1.6rem] text-[#495057] bg-white`}
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div>
                                                                <input
                                                                    type="checkbox"
                                                                    checked={employeeData?.addEmpRoleAccess?.some((item) => item.roleId === data.id)}
                                                                    onChange={(e) => handleCheckboxChange(e, data, 'roleData')}
                                                                />
                                                            </div>
                    ...
                    <div className="relative flex-1">
                        <input
                            ...
                            // onChange={handelChangeEmployeeDetails}
                            value={selectedSearchDropDownData?.defaultcentre || employeeData?.defaultcentre || ''}
                            onChange={(e) => {
                                handelChangeEmployeeDetails(e),
                                    setSelectedSearchDropDownData((preventData) => ({
                                        ...preventData,
                            ...
                            placeholder=" "
                            className={`inputPeerField peer ${formErrors.defaultcentre ? "border-b-red-500" : "border-borderColor"} focus:outline-none`}
                        />
                            ...
                            <div className="absolute border-[1px] rounded-md z-30 shadow-lg max-h-56 w-full bg-white overflow-y-auto text-xxxs">
                                <ul>
                                    {filterAllDefaultCenterData?.length > 0 ? (
                                        filterAllDefaultCenterData?.map((data, index) => (
                                            <li
                                                ...
                                                onClick={(e) => {
                                                    ...
                                                    //handelChangeEmployeeDetails(e)
                                                    handelChangeEmployeeDetails({
                                                        target: { name: 'defaultcentre', value: data?.centreId },
                                                    });
                                                }}
                                                onMouseEnter={() => setIsHovered(index)}
                                                onMouseLeave={() => setIsHovered(null)}
                                        ...
                                        <li className="py-4 text-gray-500 text-center">
                                            {import.meta.env.VITE_API_SELECT_CENTRE || 'No records found'}
                                        </li>
                                    )}
                                </ul>
                            </div>
                    ...
                    <div className="relative flex-1">
                        <input
                            ...
                            id="defaultrole"
                            name="defaultrole"
                            value={selectedSearchDropDownData?.defaultrole || employeeData?.defaultrole || ''}
                            onChange={(e) => {
                                handelChangeEmployeeDetails(e),
                                    setSelectedSearchDropDownData((preventData) => ({
                                        ...preventData,
                            ...
                            placeholder=" "
                            className={`inputPeerField peer ${formErrors.defaultrole ? "border-b-red-500" : "border-borderColor"} focus:outline-none`}
                        />
                            ...
                            <div className="absolute border-[1px] rounded-md z-30 shadow-lg max-h-56 w-full bg-white overflow-y-auto text-xxxs">
                                <ul>
                                    {filterAllDefaultRoleData?.length > 0 ? (
                                        ...
                                        <li className="py-4 text-gray-500 text-center">
                                            {import.meta.env.VITE_API_SELECT_ROLE || 'No records found'}
                                        </li>
                                    )}
                                </ul>
                            </div>
