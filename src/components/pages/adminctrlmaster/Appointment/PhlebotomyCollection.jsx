...
export default function PhlebotomyCollection() {
  ...
  const phlebotomyCollections = usePostData();
  ...
  //!==============payment data==============
  const getAmmountData = async (workOrderId, centreId) => {
  ...
  const onSubmitForDueAmt = async () => {
    ...
    try {
      const response = await phlebotomyCollections.postRequestData('/centreInvoice/create-order', updateData);
  ...
  const verifyPayment = async (response) => {
    ...
    try {
      const response = await phlebotomyCollections.postRequestData('/centreInvoice/verify-payment', updatedData);
      ...
      if (response.status === 'success') {
        ...
        const updatedData = {
          ...
          "paidAmt": duePaymentAmt?.dueAmtData
        }

        try {
          const response = await phlebotomyCollections.postRequestData('/tnx_Booking/SaveSettelmentDetail', [updatedData]);
                  ...
                  >
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        onClick={() => getAmmountData(data?.workOrderId, data?.centreId)}
                      >
                        Payment Settlement
                      </button>
                      <button
